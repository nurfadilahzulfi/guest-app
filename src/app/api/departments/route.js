...
export async function PATCH(request) {
  try {
    ...
    if (!oldName || !newName || typeof oldName !== "string" || typeof newName !== "string") {
      return Response.json({ error: "Nama lama dan nama baru departemen wajib diisi" }, { status: 400 });
    }

    const list = await masterDataService.renameDepartment(oldName, newName);
    return Response.json(list);
  } catch (error) {
